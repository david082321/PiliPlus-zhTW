...
InvalidParameter=某個無效的變數已被傳遞到了命令列:%n%n%1
...
InvalidPath=您必須輸入完整的路徑名稱及磁碟機程式碼。%n%n例如 C:\App 或 UNC 路徑格式 \\伺服器\共用資料夾。
...
; *** 壓縮文件解壓失敗詳情
ArchiveIncorrectPassword=壓縮文件密碼不正確
ArchiveIsCorrupted=壓縮文件已損壞
ArchiveUnsupportedFormat=不支援的壓縮檔案格式

; *** TDownloadWizardPage 嚮導頁面和 DownloadTemporaryFile
...
StatusCreateRegistryEntries=正在更新系統登入...
...
ErrorFunctionFailedWithMessage=%1 失敗；程式碼 %2.%n%3
ErrorExecutingProgram=無法執行檔案:%n%1

; *** Registry errors
ErrorRegOpenKey=無法開啟登錄鍵:%n%1\%2
ErrorRegCreateKey=無法建立登入項目:%n%1\%2
ErrorRegWriteKey=無法變更登入項目:%n%1\%2
...
VerificationKeyNotFound=簽名文件「%1」使用了未知金鑰
...
UninstallUnknownEntry=解除安裝記錄檔中發現未知的紀錄 (%1)。
